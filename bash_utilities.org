...
#+begin_src conf :tangle git/.gitconfig
[alias]
...
#+begin_src shell :tangle git/bin/breaksentences
sed -r -e 's/\. +/.\n/g' $*
#+end_src
...
** CHANGELOG.md "take both" merge resolution
...
Exercise caution enabling this: *all merge conflict on these files would be resolved without any prompting, which can make no sense depending on your file.*
...
#+begin_src shell :tangle bash/.jb_profile.sh
...
#+BEGIN_SRC conf :tangle emacs-daemon/.config/systemd/user/emacs.service
[Unit]
